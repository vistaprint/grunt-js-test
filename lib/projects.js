...
Project.prototype.findTests = function () {
	...
	var files = require('glob').sync(this.pattern || DEFAULT_PATTERN, {cwd: this.root})
	...
	return files.map(function (file, fileNum) {
		...
		if (isJavaScript(file)) {
			...
			url = path.join(this.baseUri, file).replace(/\\/g, '/');
		}
